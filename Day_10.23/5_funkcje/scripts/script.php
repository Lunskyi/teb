...
основываясь на идентификаторе сессии, переданном через GET- или POST-запрос, либо переданный через cookie.*/
  if (!empty($_POST)) { /*задаем условие "если переменная $_POST (дааннные поьзователя с формы) не пустая (хотя бы одно поле заполнено)"
    Так же $_POST при стандартной отправке формы не может быть пустым, он вмещает имя элементов формы даже если пользователь
    не ввел данные. по этому эта проверка для безопасности от взлома. Пустая форма можнт быть если пытаются отправить форму через исходный код*/
    //echo "<pre>";
    //print_r($_POST);
    /*echo "</pre>"; /*print_r выводит полную информацию о переменной. в данном случае это массив данных
    <pre>print_r($_POST);</pre> выводит массив в вертикальной форме*/
      ...
      */
      ...
      между двумя  переменными, $key и $value*/
        if (empty($value)) { /* задаем условие для проверки пустое ли значение, в переменной $value. Проверяем каждый
          ...
          $_SESSION['error'] = "Wypelni wszystkie pola 1"; /*$_SESSION - это сама сессия ['error'] - это имя которое мы дали
    ...
    foreach ($_POST as $key => $value) { /*включаем пошаговое перебирание всех елементов массива $_POST где
     ...
     header просто передает данные сессии дальше по адресу*/
        ...
        $_SESSION['error'] = "Wypelni formulaz";// если же переменная пустая, то есть пытаются отправить не заполненую форму через исходный код
        //echo "<script>history.back()</script>"; // возврат на страницу для заполнения формы
        //print_r($_SESSION);
        header('location: ../5.1_function_form.php');// отправка значения сессии "Wypelni formulaz" в шапку по адресу
  }
     //print_r($_SESSION);
 ?>
